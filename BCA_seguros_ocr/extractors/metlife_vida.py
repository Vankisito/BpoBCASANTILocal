...
# Suma asegurada from coberturas table
_SUMA_TABLA = r'CUBIERTO\s+([\d,]+)'
...
class MetlifeVidaExtractor(ExtractorBase):
    ...
    def extract(self, texto: str) -> dict:
        ...
        if linea_contratante:
            data['contratante_nombre'], data['poliza_numero'] = (
                self._parse_contratante(linea_contratante)
            )
        else:
        ...
        data['producto_pdf'] = (
            self._search(_PRODUCTO_ANTES, texto)
            or self._search(_PRODUCTO_DESPUES, texto)
        )
    ...
    def _parse_contratante(linea: str) -> tuple[str | None, str | None]:
        ...
        """
        m = re.search(
            r'([A-Za-z\xC0-\xD6\xD8-\xDE\xE0-\xF6\xF8-\xFF ]+?)\s+'
            r'(\d{5,10})\s*$',
            linea.strip(),
        )
        if m:
