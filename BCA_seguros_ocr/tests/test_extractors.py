...
parsers WITHOUT requiring Odoo or pypdf at runtime.
"""
...
try:
    # Odoo test runner: package imports work
    ...
    # Standalone run: add module root to sys.path
    import sys
...
def _load_fixture(poliza: str) -> str:
    ...
    # Match any fixture whose nombre starts with the póliza (with optional leading zeros)
    ...
    candidates = [
        ...
        f for f in os.listdir(FIXTURES_DIR)
        if f.startswith(poliza)
    ]
    if not candidates:
        raise unittest.SkipTest(f'Fixture not found for póliza: {poliza}')
...
class TestLayoutDetection(unittest.TestCase):
    ...
    def test_gmm_marker_medical(self):
        self.assertEqual(detectar_layout('Póliza de Seguro de:\nGASTOS MEDICOS MEDICALIFE'), 'gmm')
    ...
    def test_vida_marker_individual(self):
        self.assertEqual(detectar_layout('TEMPOLIFE POLIZA DE SEGURO VIDA INDIVIDUAL'), 'vida')
...
class TestGmmExtractor(unittest.TestCase):
...
class TestVidaExtractor(unittest.TestCase):
