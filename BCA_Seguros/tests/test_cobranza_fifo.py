...
class TestCobranzaDiaria(_CobranzaFixtures):
    ...
    def test_cinco_filas_cuatro_validas_una_no_encontrada(self) -> None:
        ...
        no_enc = bitacora.linea_ids.filtered(
            lambda linea: linea.marca == "no_encontrada"
        )
    ...
    def test_error_en_fila_no_detiene_proceso(self) -> None:
        ...
        filas = [
            ...
            self._fila_vida(
                "PV-ERR",  # fila 2 → recibo 2
                vigencia_desde="01/02/2025",
                vigencia_hasta="01/03/2025",
            ),
            self._fila_vida(
                "PV-ERR",  # fila 3 → recibo 3
                vigencia_desde="01/03/2025",
                vigencia_hasta="01/04/2025",
            ),
            self._fila_vida(
                "PV-ERR",  # fila 4 → recibo 4
                vigencia_desde="01/04/2025",
                vigencia_hasta="01/05/2025",
            ),
            self._fila_vida(
                "PV-ERR",  # fila 5 → error
                vigencia_desde="01/05/2025",
                vigencia_hasta="01/06/2025",
        ...
        linea5 = bitacora.linea_ids.filtered(lambda linea: linea.numero_fila == 5)
    ...
    def test_fifo_aplica_en_orden(self) -> None:
        ...
        filas = [
            ...
            self._fila_vida(
                "PV-FIFO", vigencia_desde="01/02/2025", vigencia_hasta="01/03/2025"
            ),  # recibo 2
            self._fila_vida(
                "PV-FIFO", vigencia_desde="01/03/2025", vigencia_hasta="01/04/2025"
            ),  # recibo 3
        ]
        ...
        pendiente_min = min(
            pol.recibo_ids.filtered(lambda r: r.estado == "pendiente").mapped(
                "numero_recibo"
            )
        )
    ...
    def test_gmm_numero_fila_preserva_posicion_csv(self) -> None:
        """Las filas válidas conservan su nº de fila real del CSV aunque haya
        anulaciones en medio (reproduce duplicado de numero_fila)."""
        for i in range(1, 5):
            self._poliza("PG-NF-%d" % i, ramo="gmm", periodicidad="mensual")
        filas = [
            self._fila_gmm("PG-NF-1"),
            self._fila_gmm("PG-NF-2", estatus_pago="anulado"),
            self._fila_gmm("PG-NF-3", estatus_pago="anulado"),
            self._fila_gmm("PG-NF-4"),
        ]
        bitacora = self._procesar(COLUMNAS_GCAYE, filas, ramo="gmm")

        self.assertEqual(bitacora.anulaciones_ignoradas, 2)
        self.assertEqual(bitacora.recibos_aplicados, 2)
        self.assertEqual(sorted(bitacora.linea_ids.mapped("numero_fila")), [1, 2, 3, 4])
        aplicadas = bitacora.linea_ids.filtered(lambda linea: linea.marca == "aplicado")
        self.assertEqual(sorted(aplicadas.mapped("numero_fila")), [1, 4])
