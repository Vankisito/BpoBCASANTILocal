...
from ..parsers import ParserMetLifeGMM, ParserMetLifeVida, ParserQualitas, get_parser
...
class TestParserBase(TransactionCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.bitacora = (
            cls.env["bca.bitacora.importacion"]
            .sudo()
            .create(
                {
    ...
    def test_validar_estructura_detecta_columnas_faltantes(self) -> None:
        incompletas = [
            c for c in self.parser.columnas_requeridas if c != "numero_poliza"
        ]
        with self.assertRaises(UserError) as ctx:
    ...
    def test_normalizar_fecha_formato_metlife(self) -> None:
        self.assertEqual(self.parser.normalizar_fecha("15/03/2026"), date(2026, 3, 15))
...
class TestParserMetLifeVida(TransactionCase):
    ...
    def test_metlife_vida_poliza_no_encontrada(self) -> None:
        ...
        for r in self.poliza.recibo_ids:
            self.assertEqual(
                r.estado, "pendiente", "Recibos intactos cuando no hay match de póliza"
            )
    ...
    def test_metlife_vida_conducto_no_match_continua(self) -> None:
        ...
        self.assertEqual(recibo.estado, "pagado")
        self.assertFalse(
            recibo.conducto_id, "conducto_id queda vacío cuando no hay match"
        )
...
class TestParserMetLifeGMM(TransactionCase):
    ...
    def test_metlife_gmm_anulacion_se_ignora(self) -> None:
        ...
        self.assertEqual(filtradas[0][1]["numero_poliza"], "POL-GMM-001")
        self.bitacora.invalidate_recordset(["anulaciones_ignoradas"])
        self.assertEqual(self.bitacora.anulaciones_ignoradas, anulaciones_antes + 1)
        ...
        self.assertTrue(lineas_anuladas, "Debe crearse línea bitácora marca=anulado")
