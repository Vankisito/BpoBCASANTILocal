...
class BcaWizardCobranzaDiaria(models.TransientModel):
    ...
    def action_procesar(self) -> dict:
        ...
        # Bajo aislamiento REPEATABLE READ el perdedor de una carga concurrente
        # aborta con 40001 (SerializationFailure) en el SELECT ... FOR UPDATE.
        # Un reintento sobre una transacción limpia ya ve el pago del ganador.
        try:
            bitacora = self._aplicar_lote()
            ...
            try:
                ...
                raise UserError(
                    _(
                        "La cobranza no pudo aplicarse por conflicto concurrente "
                        "de transacciones (reintente la carga)."
                    )
                )
        return {
    ...
    def _aplicar_lote(self):
        """Crea la bitácora, filtra (R-COB-01) y aplica cada fila FIFO en su
        propio savepoint (R-COB-08). Devuelve la bitácora generada."""
        inicio = datetime.now()
        parser_cls = get_parser(self.aseguradora_id.bca_codigo_aseguradora, self.ramo)

        bitacora = (
            self.env["bca.bitacora.importacion"]
            .sudo()
            .create(
                {
        ...
        filas = parser.filtrar_filas(list(self._abrir_csv()))
        ...
        _logger.info(
            ...
            bitacora.name,
            self.aseguradora_id.bca_codigo_aseguradora,
            self.ramo,
            len(filas),
