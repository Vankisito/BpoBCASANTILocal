...
class BcaWizardCargaPortafolio(models.TransientModel):
    ...
    def _recorrer(self, wb, dry_run: bool) -> tuple:
        ...
        if not soportadas and not tiene_beneficiarios and not tiene_coberturas:
            raise UserError(
                ...
                % (
                    ", ".join(list(HOJAS_RAMO) + [HOJA_BENEFICIARIOS, HOJA_COBERTURAS]),
                    ", ".join(wb.sheetnames),
                )
            )
        ...
        for hoja in soportadas:
            ...
            if faltantes:
                ...
                raise UserError(
                    _('La hoja "%s" no tiene las columnas requeridas: %s')
                    % (hoja, ", ".join(faltantes))
                )
            for numero_fila, raw in filas:
                ...
                resultados.append(
                    self._procesar_fila(hoja, ramo, numero_fila, raw, dry_run)
                )
        ...
        if tiene_beneficiarios:
            ...
            encabezados, filas = self._extraer_filas(ws)
            faltantes = [
                c for c in COLUMNAS_REQUERIDAS_BENEFICIARIOS if c not in encabezados
            ]
            if faltantes:
                raise UserError(
                    _('La hoja "%s" no tiene las columnas requeridas: %s')
        ...
        if tiene_coberturas:
            ...
            encabezados, filas = self._extraer_filas(ws)
            faltantes = [
                c for c in COLUMNAS_REQUERIDAS_COBERTURAS if c not in encabezados
            ]
            if faltantes:
                raise UserError(
                    _('La hoja "%s" no tiene las columnas requeridas: %s')
    ...
    def _extraer_filas(self, ws) -> tuple:
        ...
        for idx, fila_valores in enumerate(ws.iter_rows(values_only=True), start=1):
            if idx == FILA_ENCABEZADOS:
                encabezados = [
                    str(v).strip() if v is not None else "" for v in fila_valores
                ]
    ...
    # ------------------------------------------------------------------ #
    def _procesar_beneficiarios(
        self, filas: list, dry_run: bool, folios_en_archivo: dict
    ) -> tuple:
    ...
    def _procesar_grupo_beneficiarios(
        self, folio: str, items: list, dry_run: bool, folios_en_archivo: dict
    ) -> dict:
        ...
        rango = (
            "%s–%s" % (filas_num[0], filas_num[-1])
            if len(filas_num) > 1
            else str(filas_num[0])
        )
        resultado = {
            "hoja": HOJA_BENEFICIARIOS,
            "fila": rango,
            "poliza": folio or "-",
            "accion": "",
            "motivo": "",
        ...
        try:
            ...
            if not poliza and ramo is None:
                raise UserError(
                    _('No existe la póliza "%s" para esta aseguradora.') % folio
                )
                ...
                beneficiarios.append(
                    {
                        ...
                        "fecha_nacimiento": self._norm_fecha(
                            raw.get(COL_BENEF_FECHA_NAC)
                        ),
                    }
                )
            if not beneficiarios:
                raise UserError(
                    _('La póliza "%s" no tiene beneficiarios con nombre válido.')
                    % folio
                )
            ...
            if not poliza:
                raise UserError(
                    _('No existe la póliza "%s" (no se creó en esta carga).') % folio
                )
    ...
    def _procesar_grupo_coberturas(
        self,
        folio: str,
        items: list,
        dry_run: bool,
        indice: dict,
        folios_en_archivo: dict,
    ) -> dict:
    ...
    def _resolver_ptav_cobertura(self, poliza, attr, xmlid: str):
        ...
        if not poliza or not poliza.producto_id or not attr:
            return Ptav
        value = self.env.ref("BCA_Seguros.%s" % xmlid, raise_if_not_found=False)
        if not value:
            return Ptav
        return Ptav.search(
            [
    ...
    # ------------------------------------------------------------------ #
    def _procesar_fila(
        self, hoja: str, ramo: str, numero_fila: int, raw: dict, dry_run: bool
    ) -> dict:
        numero_poliza = self._txt(raw.get(COL_NUMERO_POLIZA[ramo]))
        resultado = {
            "hoja": hoja,
            "fila": numero_fila,
            "poliza": numero_poliza or "-",
            "accion": "",
            "motivo": "",
        }
        try:
            ...
            _logger.exception(
                "Error procesando fila %s de la hoja %s", numero_fila, hoja
            )
            resultado["motivo"] = _("Error inesperado: %s") % exc
        return resultado
            ...
            vals.update(
                {
                    ...
                    "recargo_fraccionamiento": self._norm_monto(
                        raw.get("Recargos (pago fraccionado)")
                    ),
    ...
    def _resolver_agente(self, clave_raw):
        ...
        if clave.isdigit():
            ...
            # '19799'). Se comparan por valor numérico; si colisionan
            # variantes históricas, gana la registrada más recientemente.
            registro = (
                Bridge.search(base)
                .filtered(
                    lambda b: (b.clave_agente or "").strip().isdigit()
                    and ((b.clave_agente or "").strip().lstrip("0") or "0")
                    == clave.lstrip("0")
                )
                .sorted(key=lambda b: b.id, reverse=True)[:1]
            )
        else:
            registro = Bridge.search(base + [("clave_agente", "=", clave)], limit=1)
        if not registro:
            raise UserError(
                _('Clave de agente "%s" no registrada en la aseguradora.') % clave
            )
    ...
    def _resolver_producto(self, nombre: str, ramo: str):
        ...
        if not producto:
            producto = Producto.search(
                dominio_base + [("name", "ilike", nombre)], limit=1
            )
        if not producto:
            raise UserError(
                _('Producto "%s" no encontrado para el ramo %s.') % (nombre, ramo)
            )
        return producto
    ...
    def _resolver_poliza(self, folio: str):
        ...
        if not folio.isdigit():
            return Poliza
        # Tolerar ceros a la izquierda (los archivos de la aseguradora traen
        # folios tipo '0008312115'): comparar por valor numérico; si colisionan
        # variantes históricas, gana la más reciente.
        objetivo = folio.lstrip("0") or "0"
        candidatos = (
            Poliza.search(
                [
                    ("aseguradora_id", "=", self.aseguradora_id.id),
            ...
            .filtered(
                lambda p: (p.name or "").isdigit()
                and ((p.name or "").lstrip("0") or "0") == objetivo
            )
    ...
    def _datos_contratante(self, raw: dict, ramo: str) -> dict:
        ...
        datos = {
            ...
            "bca_estado_civil": self._map_simple(
