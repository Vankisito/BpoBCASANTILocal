...
| Plantilla en `draft` | Filtro `('state', '=', 'draft')` → siempre se copia la plantilla original, nunca una copia ya enviada. (`limit=1` + orden por id → la de menor id, la plantilla base). |
